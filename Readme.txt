...
**I highly recommend importing the code into an IDE for review.**
There is full javadoc documentation for part one in the /doc/ directory. I suggest starting with doc/package-summary.html to view all files in part 1. *You will need to download and open these files in a web browser to read them properly.*
To test the code to make sure part 1 and two actually produce the same results you can un-comment the lines following "//TESTING" in Game.java and Driver.java, the expected output will show up in the console when run. 
